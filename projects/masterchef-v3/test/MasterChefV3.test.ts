...
describe("MasterChefV3", function () {
  ...
  describe("Real world user flow", function () {
    ...
    context.only("user reward harvesting", function () {
      it('should execute successfully', async function () {
        ...
        // Farming oasis
        let user1FarmingOasis = await this.masterChefV3.userFarmingOasis(user1.address) as boolean
        assert(!user1FarmingOasis)
        
        // Switch farming oasis
        await this.masterChefV3.connect(user1).updateFarmingOasis(true)
        await time.increase(1);

        user1FarmingOasis = await this.masterChefV3.userFarmingOasis(user1.address)
        assert(user1FarmingOasis)
        ...
        // Harvest oasis
        const tx = await this.masterChefV3.connect(user1).harvest(1, user1.address);
        // await time.increase(1);
        ...
        //   res,
        // })
        await this.masterChefV3.connect(user1).harvest(2, user1.address);
        
        // Harvest everest
        const user2EverestBalInit = await this.everestToken.balanceOf(user2.address)
        await this.masterChefV3.connect(user2).harvest(3, user2.address);
        await this.masterChefV3.connect(user2).harvest(4, user2.address);

        await time.increase(1);
        await time.increase(1);

        // Check balances
        const user1CakeBal = await this.cakeToken.balanceOf(user1.address);
        const user1EverestBal = await this.everestToken.balanceOf(user1.address);
        console.log(`user1Cake: ${ethers.utils.formatUnits(user1CakeBal)}`);
        console.log(`user1Everest: ${ethers.utils.formatUnits(user1EverestBal)}`);
        assert(user1EverestBal.eq(0))

        const user2EverestBalFinal = await this.everestToken.balanceOf(user2.address)
        const user2CakeBalFinal = await this.cakeToken.balanceOf(user2.address)
        console.log(`user2CakeFinal: ${ethers.utils.formatUnits(user2CakeBalFinal)}`);
        console.log(`user2EverestInit: ${ethers.utils.formatUnits(user2EverestBalInit)}`);
        console.log(`user2EverestFinal: ${ethers.utils.formatUnits(user2EverestBalFinal)}`);
